    ...
    class ModEntry : Mod
    {
        ...
        **********/
        public override void Entry(IModHelper helper)
        {
            ...
            PauseMode.Value = Config.Value.PauseMode_ANY_ALL_AUTO_EVENTONLY.ToUpper();

            //사용자 /명령어 추가
            helper.ConsoleCommands.Add("freeze", "Freeze the game.\n\nUsage: freeze <value>\n- value: true or false", this.SetFreezeByCommand);
        ...
        public void OnUpdateTicked(object sender, UpdateTickedEventArgs e)
        {
            if (Context.IsWorldReady && Game1.IsMultiplayer)
            {
                ...
                if (isPaused != IsPaused.Value)
                {
                    ...
                    if (PauseMode.Value != Config.Value.PauseMode_ANY_ALL_AUTO_EVENTONLY.ToUpper())
                    {
                        state.ConfigPauseMode = Config.Value.PauseMode_ANY_ALL_AUTO_EVENTONLY.ToUpper();
                        if (Context.IsMainPlayer)
        ...
        **********/
        ...
        void SetFreezeByCommand(string command, string[] args)
        {

            var state = GetPlayerState(Game1.player.UniqueMultiplayerID);
            state.IsSetFreezeByCommand = bool.Parse(args[0]);
        ...
        **********/
        ...
        class PlayerState
        {
            ...
            public bool IsSetFreezeByCommand;

            public PlayerState(bool isPaused, int ticks, int totalTimePaused = 0, bool isOnline = true, bool isHost = false, string configPauseMode = PAUSE_EVENTONLY, bool isEventing = false, bool isSetFreezeByCommand = false)
            {
                IsPaused = isPaused;
                Ticks = ticks;
                ...
                IsSetFreezeByCommand = isSetFreezeByCommand;
            }

            public override string ToString()
            {
                return $"paused: {IsPaused}, total: {TotalTimePaused}, online: {IsOnline}, host: {IsHost}, mode: {ConfigPauseMode}, eventing: {IsEventing}";
            }
        }
